	...
	if ((opt = *argv[1]) == '-')
	{
		switch (opt = *(argv[1]+1))
		{
			...
			case 'w': /* Weekday option (returns weekday abbreviation) */
				weekday_wanted = TRUE; /* Initially false, now true */
...
/*--------------------------------------------------------------------*/
{
	...
	P( "\n");
	P(" Exit Codes: For all options except weekday (-w)" "\n");
	P(" Zero = Normal | Success" "\n");
	P(" Nonzero (7+) = Abnormal | Failure" "\n");
	P(" For weekday (-w) option" "\n");
...
{
	#define P printf
	P(" Program ID: %s (05-30-2003 / v2.0)\n", progname);
	P( "\n");
	P(" Usage: %s -bHhJjstuwy\n"
	...
	P(" -s Days 'since' yyyymmdd **" "\n");
	P(" -t Tomorrow's date" "\n");
	P(" -u Basedate of today (or date specified)" "\n");
	P(" since Jan. 1, 1970--the Unix epoch date" "\n");
	P(" (essentially 'dateplus -s 19700101')" "\n");
	P(" -w Date's weekday (see 'Exit Codes' below)" "\n");
	P(" -y Yesterday's date" "\n");
	P( "\n");
	P(" * 'adj_days' may be positive, negative, or" "\n");
	P(" unsigned." "\n");
	P( "\n");
	P(" 'adj_days' by itself (without a following" "\n");
	P(" yyyymmdd date), yields an adjusted date" "\n");
	P(" that is calculated from today's date. For" "\n");
	P(" example, '%s 1' yields tomorrow's date).\n",
		progname);
	P( "\n");
	P(" * 'yyyymmdd' is the optional date from which" "\n");
	P(" an adjusted date is calculated." "\n");
	P( "\n");
	P(" * 'iterations' generate multiple dates that" "\n");
	P(" are 'adj_days' apart (i.e. paydays). This""\n");
	P(" argument is not allowed with the julian" "\n");
	P(" (-j) option." "\n");
	P( "\n");
	P(" ** yyyymmdd may be a future date (useful for" "\n");
	P(" determining the number of days until a" "\n");
	P(" certain date). The number displayed is" "\n");
	P(" negative. For example, run on October 27,""\n");
	P(" 1997), the command 'basedate -s 20000101'" "\n");
	P(" yielded -731, meaning 731 days until Y2K.\n\n");
	#undef P
	return;
}
/*--------------------------------------------------------------------*/
